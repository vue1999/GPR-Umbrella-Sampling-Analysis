...
from .plotting_1d import plot_diagnostics, apply_plot_style, PALETTE
from .projected_inputs import prepare_projected_observations
from .robust_1d import fit_linear_observations, barrier_posterior, profile_landmarks
...
def load_inputs(colvar_dir, kappa_dir, reference, cv_cols=(1, 2)):
    ...
    for i in sorted(colvars):
        ...
        if len(times) < 4 or np.any(differences <= 0) or not np.allclose(differences, np.median(differences), rtol=1e-3):
            raise ValueError(f"Window {i}: time must be increasing and uniformly spaced")
        ...
        provenance.append({"window_id": i, "colvar": str(colvars[i].resolve()),
                           "colvar_sha256": hashlib.sha256(colvars[i].read_bytes()).hexdigest(),
                           "restraint_sha256": hashlib.sha256(kappafiles[i].read_bytes()).hexdigest(),
                           "samples": len(raw), "time_start": times[0], "time_end": times[-1]})
    ...
    return trajectories, restraints[:, :2], restraints[:, 2:], vertices, provenance, dt[0]
...
def _quality_plot(prepared, result, output):
    ...
    scatter = axes[0, 0].scatter(table[:, 0], table[:, 1], c=table[:, -1], cmap="viridis", s=20)
    axes[0, 0].set(xscale="log", yscale="log", xlabel="SE lengthscale (Å)", ylabel="SE amplitude (eV)", title="Hyperparameter posterior weights")
    ...
    axes[0, 1].set(xlabel="Window", ylabel="Window", title=f"Original 2D-bias overlap: {prepared['overlap_components']} components")
    axes[1, 0].plot(prepared["nodes"], prepared["block_ess"], color=PALETTE["sampling"])
    axes[1, 0].plot(prepared["nodes"], prepared["unweighted_block_ess"], color=PALETTE["guide"], ls="--", label="Unweighted blocks")
    axes[1, 0].axhline(4, ls="--", color=PALETTE["warn"])
    axes[1, 0].legend()
    axes[1, 0].set(xlabel="Arclength (Å)", ylabel="Effective contributing time blocks", title="Reweighted support per bin")
    for half, profile in enumerate(prepared["half_profiles"]):
        axes[1, 1].plot(prepared["nodes"], profile, label=f"Time half {half + 1}")
    axes[1, 1].plot(prepared["nodes"], prepared["histogram_pmf"], color=PALETTE["pmf"], label="Full trajectory", lw=2)
    axes[1, 1].set(xlabel="Arclength (Å)", ylabel="ΔF (eV)", title="Time-split convergence (same reference bin)")
    axes[1, 1].legend()
...
def run(args):
    ...
    code_provenance = {p.name: hashlib.sha256(p.read_bytes()).hexdigest()
                       for p in sorted(Path(__file__).parent.glob("*.py"))}
    trajectories, centers, kappas, vertices, provenance, dt = load_inputs(args.colvar_dir, args.kappa_dir, args.path_reference)
    campaign_root = Path(args.colvar_dir).parent
    manifest, path_xyz = campaign_root / "source_campaign.json", campaign_root / "neb_path_30_images.xyz"
    if manifest.is_file() and path_xyz.is_file():
        ...
        if not np.isclose(campaign_audit["temperature_K"], args.temperature, rtol=0, atol=1e-8):
    ...
    prepared = prepare_projected_observations(trajectories, centers, kappas, vertices,
    ...
    result = fit_linear_observations(nodes, prepared["operator"], prepared["values"],
                                    prepared["noise_covariance"], grid, resolution=resolution)
    issues = prepared["quality_issues"] + result["quality_issues"] + campaign_audit["quality_issues"]
    ...
    if args.reactant_interval is not None or args.transition_interval is not None:
        ...
        barrier = barrier_posterior(result, args.reactant_interval, args.transition_interval, seed=args.seed)
        if barrier["std"] > args.max_barrier_std:
            issues.append("barrier_uncertainty_exceeds_target"); status = "UNRELIABLE"
        if max(barrier["reactant_extremum_boundary_probability"], barrier["transition_extremum_boundary_probability"]) > .5:
            issues.append("barrier_extremum_at_interval_boundary"); status = "UNRELIABLE"
    else:
    ...
    for i, p in enumerate(positions):
        if p.var(ddof=1) > 0:
            for blocks in (8, 16, 32, 64):
                ...
                tau[i] = max(tau[i], len(p) * means.var(ddof=1) / (2 * blocks * p.var(ddof=1)))
    result.update({"x_centers": prepared["center_s"], "x_means": np.array([p.mean() for p in positions]),
                   "x_vars": np.array([p.var(ddof=1) for p in positions]),
                   "n_samples": np.array([len(p) for p in positions]), "tau_ints": tau,
                   ...
                   "training_std_residuals": result["training_residuals"] / np.sqrt(np.diag(prepared["noise_covariance"])),
                   "all_positions": [p[::max(1, len(p)//2000)] for p in positions],
                   "kappa": kappas, "cv_unit": "Å", "energy_unit": "eV", "deriv_unit": "eV/Å",
                   "uncertainty_calibration_factor": None, "quality_status": status})
    np.savetxt(output / "pmf_1d.dat", np.c_[grid, result["pmf_mean"], result["pmf_std"]],
    ...
    np.savez_compressed(output / "fit_arrays.npz", nodes=nodes, bin_edges=prepared["bin_edges"], operator=prepared["operator"],
               ...
               block_ess=prepared["block_ess"], unweighted_block_ess=prepared["unweighted_block_ess"],
               relative_block_ess=prepared["relative_block_ess"], half_profiles=prepared["half_profiles"])
    summary = {"status": status, "quality_issues": issues, "barrier": barrier,
               ...
               "overlap_components": prepared["overlap_components"], "overlap_scalar": prepared["overlap_scalar"],
               "minimum_effective_blocks": float(prepared["block_ess"].min()),
               "minimum_relative_effective_blocks": float(prepared["relative_block_ess"].min()),
               "half_profile_difference_range_eV": prepared["half_profile_difference_range"],
               "settings": vars(args), "campaign_audit": campaign_audit, "stored_sample_interval_fs": dt,
               "code_sha256": code_provenance, "python": sys.version,
               "dependencies": {name: version(name) for name in ("numpy", "scipy", "pymbar", "matplotlib")},
    ...
    print(json.dumps({k: summary[k] for k in ("status", "quality_issues", "lengthscale_A", "loo_rms", "barrier")}, indent=2), flush=True)
    return summary
...
def main():
    ...
    parser.add_argument("--path-reference", required=True, help="Ordered two-column HH/RELZ path in Å")
    ...
    parser.add_argument("--max-barrier-std", type=float, default=.05, help="Reporting target in eV; never used to shrink errors")
    ...
    try:
        ...
        output = Path(args.output_dir)
        output.mkdir(parents=True, exist_ok=True)
        (output / "failure.json").write_text(json.dumps({"status": "FAILED", "reason": str(error), "settings": vars(args)}, indent=2) + "\n")
        raise
