...
def audit_campaign(manifest, path_xyz, *, fixed_tolerance=1e-6):
    ...
    same_atoms = all(a.get_chemical_symbols() == frames[0].get_chemical_symbols() for a in frames)
    ...
    maximum = float(np.max(displacement)) if displacement.size else 0.
    issues = []
    if not same_atoms:
    ...
    return {"quality_issues": issues, "max_fixed_atom_displacement_A": maximum,
            "n_windows": meta["n_windows"], "temperature_K": meta["sampling"]["temperature_K"],
            "model_sha256": meta.get("inputs", {}).get("model_sha256"),
            "fixed_atom_tolerance_A": fixed_tolerance, "same_atom_order": same_atoms,
            "same_cell": same_cell, "manifest": str(manifest), "path_xyz": str(path_xyz)}
