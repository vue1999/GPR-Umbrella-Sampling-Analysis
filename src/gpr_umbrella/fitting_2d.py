...
def _se(Xa: np.ndarray, Xb: np.ndarray, sigma_f: float, ell: np.ndarray) -> np.ndarray:
...
def _k_f_grad(
    Xs: np.ndarray, Xt: np.ndarray, sigma_f: float, ell: np.ndarray
) -> np.ndarray:
...
def _k_grad_grad(
    Xa: np.ndarray, Xb: np.ndarray, sigma_f: float, ell: np.ndarray
) -> np.ndarray:
...
def training_covariance(signal, sampling_cov, y, extra_noise=None):
    ...
    covariance[np.diag_indices_from(covariance)] += jitter + np.tile(
        eta**2, len(y) // 2
    )
    return covariance
...
def objective(log_parameters, X, y, sampling_cov, noise_scale=None):
    ...
    value = 0.5 * y @ alpha + np.log(np.diag(cf[0])).sum() - normalization
    ...
    for d in range(2):
        ...
        correction[d, d] = -2 * inv[d]
        indicator = np.zeros(2)
        indicator[d] = 1
        term += correction + 2 * product * (indicator[:, None] + indicator[None, :])
        derivatives.append(
            (k[:, :, None, None] * term).transpose(0, 2, 1, 3).reshape(len(y), len(y))
        )
    gradient = [0.5 * np.sum(W * derivative) for derivative in derivatives]
    if extra:
        prior = (eta / noise_scale) ** 2
        value += 0.5 * prior.sum()
        gradient.extend(
            eta[d] ** 2 * np.diag(W)[d::2].sum() + prior[d] for d in range(2)
        )
    return float(value), np.asarray(gradient)


def fit_hyperparameters(
    X, y, sampling_cov, ell_init, ell_upper, sigma_f_init,
    fixed_lengthscale, fixed_sigma_f, optimize, *,
    fit_extra_noise=False, extra_noise_scale=None, sigma_f_max=None,
):
    ...
    if fixed_lengthscale is not None:
        fixed_ell = np.broadcast_to(np.asarray(fixed_lengthscale, float), (2,)).copy()
    positive_parameters = {
        "initial lengths": ell_init, "upper lengths": ell_upper,
        "initial amplitude": sigma_f_init, "fixed lengths": fixed_ell,
        "fixed amplitude": fixed_sigma_f, "amplitude cap": sigma_f_max,
    }
    for name, values in positive_parameters.items():
        if values is not None and (
            np.any(~np.isfinite(values)) or np.any(np.asarray(values) <= 0)
        ):
    ...
    base = np.r_[sigma_f_init, ell_init]
    lower = np.r_[sf_lower, ell_upper / 3000.]
    upper = np.r_[sf_upper, ell_upper]
    if fit_extra_noise:
        base = np.r_[base, scale]
        lower = np.r_[lower, scale * 1e-6]
        upper = np.r_[upper, scale * 5.]
    free = np.array(
        ([0] if fixed_sigma_f is None else [])
        + ([1, 2] if fixed_ell is None else [])
        + ([3, 4] if fit_extra_noise else []), int,
    )
    initial = base.copy()
    if fixed_sigma_f is not None:
        initial[0] = fixed_sigma_f
    if fixed_ell is not None:
        initial[1:3] = fixed_ell
    if not optimize or not len(free):
        ...
        metadata = dict(
            trials=[], extra_noise_scale=scale if fit_extra_noise else None,
            sigma_f_max=sigma_f_max, bound_hits=[],
        )
        return initial[0], initial[1:3], np.zeros(2), True, metadata
    ...
    for sf_ratio, ell_ratio, noise_ratio in starts:
        ...
        result = minimize(
            fun, ratios[free], jac=True, method="L-BFGS-B", bounds=bounds[free],
            options=dict(maxiter=600, ftol=1e-11, gtol=1e-6, maxls=50),
        )
        final = ratios.copy()
        final[free] = result.x
        parameters = base * np.exp(final)
        trials.append(dict(
            parameters=parameters.tolist(), objective=float(result.fun),
            success=bool(result.success), iterations=int(result.nit),
            message=str(result.message),
        ))
    valid = [
        t for t in trials
        if t["success"] and np.isfinite(t["objective"]) and t["objective"] < 1e90
    ]
    if not valid:
        raise RuntimeError("All six GP optimization starts failed; no initial-guess fit returned")
    best = min(valid, key=lambda t: t["objective"])
    p = np.array(best["parameters"])
    names = ["sigma_f", "ell0", "ell1", "extra_noise0", "extra_noise1"]
    hits = [
        names[i] + ("_lower" if p[i] <= lower[i] * 1.001 else "_upper")
        for i in free if p[i] <= lower[i] * 1.001 or p[i] >= upper[i] / 1.001
    ]
    metadata = dict(
        trials=trials, objective=best["objective"],
        ...
        objective_definition=("NLL minus fixed unit-normalization constant "
                              "plus optional half-normal noise penalty"),
    )
