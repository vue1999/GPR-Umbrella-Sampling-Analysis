...
from .integration_1d import k_base, k_f_fprime, k_fprime_fprime
...
def fit_linear_observations(nodes, operator, values, noise_covariance, grid, *,
                            resolution=None, lengthscales=None, amplitudes=None):
    ...
    n = len(values)
    if (nodes.ndim != 1 or grid.ndim != 1 or len(nodes) < 3 or n < 2
            or np.any(np.diff(nodes) <= 0) or np.any(np.diff(grid) <= 0)
            or operator.shape != (n, len(nodes)) or noise.shape != (n, n)
            or not all(np.all(np.isfinite(a)) for a in (nodes, values, grid, operator, noise))):
        raise ValueError("Invalid GP observations, covariance, or ordered coordinates")
    if not np.allclose(noise, noise.T) or np.linalg.eigvalsh(noise).min() < -1e-10 * np.max(np.diag(noise)):
    ...
    span = np.ptp(nodes)
    resolution = float(resolution if resolution is not None else np.median(np.diff(nodes)))
    ...
    if amplitudes is None:
        ...
        scale = max(signal, np.sqrt(np.mean(np.diag(noise))) / np.linalg.norm(operator, axis=1).mean())
        amplitudes = np.geomspace(scale / 30, scale * 30, 25)
    lengthscales, amplitudes = [np.asarray(v, dtype=float) for v in (lengthscales, amplitudes)]
    if any(v.ndim != 1 or len(v) == 0 or np.any(~np.isfinite(v)) or np.any(v <= 0)
    ...
    for axis in (lengthscales, amplitudes):
        if len(axis) > 2 and not np.allclose(np.diff(np.log(axis)), np.diff(np.log(axis))[0]):
            raise ValueError("Use log-uniform hyperparameter grids")
    records = []
    jitter = np.mean(np.diag(noise)) * 1e-9
    for ell in lengthscales:
        base = operator @ k_base(nodes, nodes, 1., ell) @ operator.T
        for amplitude in amplitudes:
            ...
            try:
                ...
                continue
            alpha = inv @ values
            nll = .5 * (values @ alpha + 2 * np.log(np.diag(factor[0])).sum() + n * np.log(2 * np.pi))
            loo_var = 1 / np.diag(inv)
            loo_z = alpha * np.sqrt(loo_var)
            records.append((float(ell), float(amplitude), float(nll), float(np.sqrt(np.mean(loo_z**2))), float(np.max(np.abs(loo_z)))))
    if not records:
    ...
    mean = np.zeros(len(grid)); second = np.zeros((len(grid), len(grid)))
    dmean = np.zeros(len(grid)); dsecond = np.zeros(len(grid))
    components = []
    best_result = None
    for idx, weight in zip(selected, weights_retained):
        ...
        ky = kernel + noise + np.eye(n) * jitter
        factor = cho_factor(ky, lower=True)
        inv = cho_solve(factor, np.eye(n)); alpha = inv @ values
        cross = k_base(grid, nodes, amplitude, ell) @ operator.T
        fm = cross @ alpha
        fc = k_base(grid, grid, amplitude, ell) - cross @ cho_solve(factor, cross.T)
        fm -= fm[0]; fc = covariance_reference(fc)
        dcross = -k_f_fprime(grid, nodes, amplitude, ell) @ operator.T
        dm = dcross @ alpha
        dv = amplitude**2 / ell**2 - np.einsum("ij,ji->i", dcross, cho_solve(factor, dcross.T))
        mean += weight * fm; second += weight * (fc + np.outer(fm, fm))
        dmean += weight * dm; dsecond += weight * (np.maximum(dv, 0) + dm**2)
        components.append({"weight": float(weight), "mean": fm, "covariance": fc,
                           "lengthscale": float(ell), "sigma_f": float(amplitude)})
        if idx == best:
            loo_var = 1 / np.diag(inv)
            best_result = {"loo_means": values - alpha * loo_var,
                           "loo_stds": np.sqrt(loo_var), "loo_z": alpha * np.sqrt(loo_var),
    ...
    issues = []
    boundary_mass = float(weights[(table[:, 0] == lengthscales[0]) | (table[:, 0] == lengthscales[-1])].sum())
    amplitude_boundary_mass = float(weights[(table[:, 1] == amplitudes[0]) | (table[:, 1] == amplitudes[-1])].sum())
    if len(lengthscales) > 1 and boundary_mass > .2:
        issues.append("lengthscale_prior_boundary")
    if len(amplitudes) > 1 and amplitude_boundary_mass > .2:
    ...
    ky = operator @ k_base(nodes, nodes, amplitude, ell) @ operator.T + noise + np.eye(n)*jitter
    ...
    return {"x_star": grid, "pmf_mean": mean, "pmf_covariance": covariance,
            "pmf_std": np.sqrt(np.maximum(np.diag(covariance), 0)),
            "deriv_mean": dmean, "deriv_std": np.sqrt(np.maximum(dsecond - dmean**2, 0)),
            "sigma_f": float(table[best, 1]), "lengthscale": float(table[best, 0]),
            "hyperparameter_table": np.column_stack([table, weights]),
            "hyperparameter_columns": ["lengthscale", "sigma_f", "nll", "loo_rms", "loo_max_abs", "weight"],
            "components": components, "quality_issues": issues,
            "blocked_cv_rms": blocked, "lengthscale_boundary_mass": boundary_mass,
            "amplitude_boundary_mass": amplitude_boundary_mass,
            "retained_hyperparameter_mass": float(weights[selected].sum()), **best_result}


def barrier_posterior(result, reactant_interval, transition_interval, *, draws=1000, seed=2026):
    ...
    """
    x = result["x_star"]
    masks = [(x >= lo) & (x <= hi) for lo, hi in (reactant_interval, transition_interval)]
    if any(not m.any() for m in masks) or reactant_interval[1] >= transition_interval[0]:
        raise ValueError("Provide non-overlapping ordered reactant and transition intervals on the grid")
    if any(lo < x[0] or hi > x[-1] or lo >= hi for lo, hi in (reactant_interval, transition_interval)):
    ...
    barriers = []; component_barriers = []; reactant_edges = []; transition_edges = []
    for c, count in zip(components, allocation):
        component_barriers.append(float(c["mean"][masks[1]].max() - c["mean"][masks[0]].min()))
        if count == 0:
            continue
        cov = c["covariance"][np.ix_(selected, selected)]
        eig, vec = np.linalg.eigh((cov + cov.T) / 2)
        samples = c["mean"][selected] + rng.normal(size=(count, len(eig))) @ (vec * np.sqrt(np.maximum(eig, 0))).T
        reactant_samples, transition_samples = samples[:, rmask], samples[:, tmask]
        ri, ti = np.argmin(reactant_samples, axis=1), np.argmax(transition_samples, axis=1)
    ...
    return {"mean": float(barriers.mean()), "std": float(barriers.std(ddof=1)),
            "median": float(np.median(barriers)), "ci95": np.quantile(barriers, [.025, .975]).tolist(),
            "posterior_mean_profile_barrier": float(result["pmf_mean"][masks[1]].max() - result["pmf_mean"][masks[0]].min()),
            "hyperparameter_barrier_range": [min(component_barriers), max(component_barriers)],
            "reactant_interval": list(reactant_interval), "transition_interval": list(transition_interval),
...
def profile_landmarks(result, *, prominence=.02):
    ...
    for ts in maxima:
        ...
        pairs.append({"reference_s": float(x[reference]), "maximum_s": float(x[ts]),
                      "rise": float(f[ts] - f[reference]), "conditional_sigma": float(np.sqrt(max(variance, 0))),
    ...
    return {"minima_s": x[minima].tolist(), "maxima_s": x[maxima].tolist(),
            "candidate_rises": pairs, "prominence": prominence,
            "forward_rise_from_first_supported_bin": float(f[maximum] - f[0]),
            "forward_rise_conditional_sigma": float(np.sqrt(max(cov[maximum, maximum] + cov[0, 0] - 2*cov[maximum, 0], 0))),
