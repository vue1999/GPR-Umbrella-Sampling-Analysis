...
def validate_path(vertices):
    ...
    if len(delta) > 1:
        ...
        if np.any(cosine < -1 + 1e-10):
            raise ValueError(
                "Self-intersecting path: adjacent segments retrace one another"
            )
    ...
    for i, a in enumerate(delta):
        for j in range(i + 2, len(delta)):
            ...
            if intersects:
                raise ValueError(
                    f"Self-intersecting path at segments {i}, {j}; supply another CV or separate pathways"
                )
...
def project_polyline(
    ...
    *,
    ...
    for start in range(0, len(points), chunk_size):
        ...
        ambiguous = np.any(
            remote & (distance <= d[:, None] + ambiguity_distance), axis=1
        )
...
def project_arclength(
    ...
    *,
    ...
    delta = np.diff(vertices, axis=0)
    lengths = np.linalg.norm(delta, axis=1)
    h = float(0.5 * np.median(lengths) if smoothing_width is None else smoothing_width)
    if not np.isfinite(h) or h <= 0:
        raise ValueError("Smoothing width must be positive")
    tangents = delta / lengths[:, None]
    starts = hard["vertex_s"][:-1]
    soft_s = np.empty(len(points))
    remote_mass = np.empty(len(points))
    separation = max(
        4 * h,
        branch_separation if branch_separation is not None else 3 * np.median(lengths),
    )
    for begin in range(0, len(points), chunk_size):
        ...
        log_phi_a = -0.5 * alpha**2 - 0.5 * np.log(2 * np.pi)
        log_phi_b = -0.5 * beta**2 - 0.5 * np.log(2 * np.pi)
        mean_local = t + h * (np.exp(log_phi_a - log_z) - np.exp(log_phi_b - log_z))
        means = starts + mean_local
        log_mass = -0.5 * normal2 / h**2 + log_z
    ...
    if not np.all(np.isfinite(soft_s)):
        raise ValueError(
            "Non-finite soft projection; inspect path scale and smoothing width"
        )
    return {
        **hard,
        "hard_s": hard["s"],
        "s": soft_s,
        "ambiguous": hard["ambiguous"] | (remote_mass > 0.05),
