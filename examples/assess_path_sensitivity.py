...
def main():
    ...
    for root in sorted(output.glob("s*")):
        ...
        trajectories, _, _, vertices, _, _ = load_inputs(
            source / "COLVAR", source / "window_kappa", source / "neb_HH_RELZ_30img.dat"
        )
        position_cache = {}
        ...
        if not base_summary_path.is_file():
            ...
            continue
        smooth = (
            json.loads(base_summary_path.read_text()).get("projection_method") == "soft"
        )
        variants = ["base", "long_blocks", "fine_bins", "fine_stride"]
        if (root / "short_blocks").exists() or (root / "coarse_stride").exists():
            variants = ["base", "short_blocks", "fine_bins", "coarse_stride"]
        if smooth:
        ...
        for variant in variants:
            ...
            (
                coordinate_profiles if variant.startswith("projection_") else profiles
            ).append(profile)
            summary["variant"] = variant
            summaries.append(summary)
            issues.extend(
                i
                for i in summary["quality_issues"]
                if i
                not in ("sensitivity_suite_required", "low_effective_block_support")
            )
            key = (
                summary.get("projection_method", "polyline"),
                summary.get("smoothing_width_A"),
            )
            if key not in position_cache:
                position_cache[key] = [
                    project_arclength(
                        q, vertices, method=key[0], smoothing_width=key[1]
                    )["s"]
            ...
            edges = (
                ...
                if "bin_edges" in data
                    ...
                    0,
                    np.linalg.norm(np.diff(vertices, axis=0), axis=1).sum(),
                    len(data["nodes"]) + 1,
                )
            )
            baseline = unweighted_block_support(
                positions,
                edges,
                block_size=summary["settings"]["block_size"],
                stride=summary["settings"]["stride"],
            )
            ratio = data["block_ess"] / baseline
            summary["minimum_relative_effective_blocks"] = float(ratio.min())
            if data["block_ess"].min() < 4 or ratio.min() < 0.25:
        ...
        lower = table[:, 0].min()
        base_refit = fit_linear_observations(
            base["nodes"],
            base["operator"],
            base["values"],
            base["noise_covariance"],
            base["grid"],
            resolution=lower,
        )
        hypers = []
        for factor in (1.5, 2.0):
            fit = fit_linear_observations(
                ...
                amplitudes=np.geomspace(
                    table[:, 1].min() / 3, table[:, 1].max() * 3, 25
                ),
            )
            excluded_mass = float(table[table[:, 0] < lower * factor, -1].sum())
            eligible = excluded_mass <= 0.05
            if eligible:
                profiles.append(fit)
                issues.extend(fit["quality_issues"])
            hypers.append(
                {
                    "lengthscale_lower_factor": factor,
                    "baseline_posterior_mass_excluded": excluded_mass,
                    "included_in_acceptance": eligible,
                    "interpretation": "Sensitivity check"
                    if eligible
                    else "Forced-prior stress test: removes substantial supported model mass",
                    "quality_issues": fit["quality_issues"],
                    "lengthscale": fit["lengthscale"],
                    "sigma_f": fit["sigma_f"],
                    "profile_comparison": compare_profiles([profiles[0], fit]),
                }
            )
        # Test wider support and quadrature refinement without excluding the
        # models the data actually support. Arbitrary forced oversmoothing is
        # not a requirement for a correct estimator to be invariant.
        quadrature = []
        for name, extend in (("refined_grid", False), ("wider_priors", True)):
            fit = fit_linear_observations(
                base["nodes"],
                base["operator"],
                base["values"],
                ...
                lengthscales=np.geomspace(
                    lower, table[:, 0].max() * (2 if extend else 1), 70
                ),
                amplitudes=np.geomspace(
                    table[:, 1].min() / (3 if extend else 1),
                    table[:, 1].max() * (3 if extend else 1),
                    50,
                ),
            )
            profiles.append(fit)
        ...
        coordinate_comparison = (
            compare_profiles([profiles[0], *coordinate_profiles])
            if coordinate_profiles
            else None
        )
        if (
            coordinate_comparison
            and coordinate_comparison["maximum_profile_spread"] > 0.05
        ):
            issues.append("coordinate_definition_sensitivity")
        source = Path(args.data_root) / root.name
        audit = audit_campaign(
            source / "source_campaign.json", source / "neb_path_30_images.xyz"
        )
        ...
        report = {
            ...
            "prior_sensitivity": hypers,
            "hyperparameter_grid_and_prior_checks": quadrature,
            "forced_prior_rule": "Lower-bound stress tests are acceptance checks only when they remove <=5% of baseline posterior mass; all are reported",
            ...
            "variants": [
                {
                    ...
                    for k in (
                        ...
                        "minimum_relative_effective_blocks",
                        "half_profile_difference_range_eV",
                    )
                }
                for s in summaries
            ],
